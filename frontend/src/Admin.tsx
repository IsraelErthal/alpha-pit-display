...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (u) => {
      ...
      // Força atualizar o token para pegar a claim admin mais recente
      const tokenResult = await u.getIdTokenResult(true);
      const admin = tokenResult.claims.admin === true;
      if (!admin) {
        setLoginError('Este e-mail não tem permissão de administrador.');
        await signOut(auth!);
        setUser(null); setIsAdmin(false); setAuthChecked(true);
        return;
      }
    ...
    return (
      <main className="admin-page admin-gate">
        ...
        <section className="admin-login-box">
          ...
          <p>Entre com uma conta autorizada do Google para continuar.</p>
