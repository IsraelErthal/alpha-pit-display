...
app.post('/api/comments', rateLimit({ windowMs: 60_000, max: 8 }), authenticate, async (req, res, next) => {
  try { const comment = await comments.create(req.user, req.body); req.io.emit('comment:created', comment); res.status(201).json(comment); } catch (error) { next(error); }
});
...
app.patch('/api/admin/comments/:id/restore', authenticate, requireAdmin, async (req, res, next) => { try { const restored = await comments.restore(req.params.id); req.io.emit('comments:ready'); res.json(restored); } catch (error) { next(error); } });
...
app.use((error, _req, res, _next) => { if (error.name === 'ZodError') return res.status(400).json({ error: 'Comentário inválido.' }); console.error(error); return res.status(500).json({ error: 'Erro interno.' }); });
